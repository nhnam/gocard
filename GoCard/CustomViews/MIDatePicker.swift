...
class MIDatePicker: UIView {
    ...
    // MARK: - Config
    struct Config {
        let contentHeight: CGFloat = 250.0
        let bouncingOffset: CGFloat = 20.0
        var startDate: Date?
    ...
    static func getFromNib() -> MIDatePicker {
        return UINib.init(nibName: String(describing: MIDatePicker.self), bundle: nil).instantiate(withOwner: self, options: nil).last as! MIDatePicker
    }
    ...
    private func move(goUp isGoUp: Bool) {
        bottomConstraint.constant = isGoUp ? config.bouncingOffset : config.contentHeight + config.headerHeight
    }
    ...
    func show(inVC parentVC: UIViewController, completion: (() -> ())? = nil) {
        ...
        UIView.animate( withDuration: config.animationDuration, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 5, options: .curveEaseIn, animations: {
