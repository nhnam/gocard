...
//            }
    ...
    fileprivate func drawHighlightLineFromLeftPosition(_ left: CGFloat, fromTop top: CGFloat) {
        if let shapeLayer = highlightShapeLayer {
            // Use line already created
            let path = CGMutablePath()
            
            // draw duong thang dung
            path.move(to: CGPoint(x: left, y: 0))
            path.addLine(to: CGPoint(x: left, y: drawingHeight + topInset))
            
            // draw duong nam ngang
            let scaled = scaleValuesOnYAxis([Float(top)])
            let y = CGFloat(scaled.first!) - CGFloat(drawingHeight) -  topInset
            if y > CGFloat(min.y) && y <= CGFloat(drawingHeight + topInset) {
            ...
            path.addLine(to: CGPoint(x : drawingWidth + topInset, y: -top))
    ...
    func handleTouchEvents(_ touches: Set<UITouch>, event: UIEvent!) {
        ...
        let top = point.location(in: self).y
        let xValue = valueFromPointAtX(left)
        let yValue = valueFromPointAtY(top)
        ...
        drawHighlightLineFromLeftPosition(left, fromTop: CGFloat(yValue))
        ...
        delegate!.didTouchChart(self, indexes: indexes, x: xValue, left: left)
